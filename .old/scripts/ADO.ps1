...
$token = [System.Convert]::ToBase64String([System.Text.Encoding]::ASCII.GetBytes(":$($personalAccessToken)"))
$header = @{authorization = "Basic $token"}
[string]$organization = "klp-proto"  # the name of your DevOps organization
[string]$project = "KLP%20Proto" #Project Queried
[string]$apiVersion = "7.0"
[string]$packageBaseUrl = "https://pkgs.dev.azure.com/${organization}/${project}/_apis/packaging/feeds"
[string]$feedId = "klp-proto-artifacts"
[string]$packageName = "klp-proto-libs"
...
[string]$project = "KLP%20Proto" #Project Queried
$body = @{
    "name" = "klp-proto-artifacts"
}  | ConvertTo-Json -Depth 5
$url = "https://feeds.dev.azure.com/${organization}/${project}/_apis/packaging/feeds?api-version=${apiVersion}-preview"
Invoke-RestMethod -Uri $url -headers $header -Method POST -Body $body -ContentType "application/json"

#=== Get Work Item ===

[string]$id = "1678"
$url = "https://dev.azure.com/${organization}/${project}/_apis/wit/workitems/${id}?api-version=${apiVersion}"
$result = Invoke-RestMethod -Uri $url -headers $header -Method Get
...
$body = ConvertTo-Json @(
    @{
        "op" = "add"
        "path" = "/fields/Custom.UIUX"
        "value" = "iframe width=""800"" height=""450"" src=""https://www.figma.com/embed?embed_host=share&url=https%3A%2F%2Fwww.figma.com%2Ffile%2FmqGUZv3miV5bBUt2dUhbuP%2FFigma-to-Azure-Devops-(Community)%3Fnode-id%3D2%253A4"" allowfullscreen></iframe>"
    }
    # @{
    #     "op" = "add"
...
$url = "https://dev.azure.com/${organization}/${project}/_apis/wit/workitems/${id}?api-version=${apiVersion}"
$result = Invoke-RestMethod -Uri $url -headers $header -Method PATCH -Body $body -ContentType "application/json-patch+json"
