...
public partial class Downstream
{
    ...
    {
        _entraId = ConfigurationManager.GetSection("EntraId").Get<EntraIdOptions>();
        _keyValues.Add(new KeyValues(nameof(_entraId), _entraId));

        _microsoftGraph = ConfigurationManager.GetSection("DownstreamApis:MicrosoftGraph").Get<MicrosoftGraphOptions>();
